                ...
                using (IDbConnection con = new SqlConnection(Extensions.GetBD_CFDI()))
                {
                    try
                    {
                        ...
                        if( req.rep == 1 )
                        {
                            ...
                            FROM dbo.CFDI_XML_PSI xml
                            INNER JOIN " + emp.ICG_BD + @".dbo.IT_RELFACTURAS_COMPRAS it
                            ...
                            SELECT 
                                DISTINCT
                                  ...
                                  case 
                                    ...
                                    it.NUMFAC
                            FROM dbo.CFDI_XML_PSI xml
                            --INNER JOIN " + emp.ICG_BD + @".dbo.IT_RELFACTURAS_COMPRAS it ON xml.UUID = it.FOLIOFISCAL
                            INNER JOIN
                                     (SELECT DISTINCT 
                                        CAST(FECHA_PROCESO AS DATE) FECHA_PROCESO,FOLIOFISCAL,NUMSERIEFAC,NUMFAC
                                        FROM " + emp.ICG_BD + @".dbo.IT_RELFACTURAS_COMPRAS) it ON xml.UUID = it.FOLIOFISCAL
                        ...
                        if (req.rep == 3)
                        {
                            ...
                            foreach (var rs in xmls)
                            {
                                 try
                                 {
                                      ...
                                      var xml = con.QueryFirst<string>("SELECT XML FROM dbo.CFDI_XML_PSI WHERE UUID = @UUID", new { rs.UUID });
