...
namespace WB_CFDI_V1.Vistas
{
    public partial class excel3 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            List<int> data1 = (List<int>)HttpContext.Current.Session["data1"];
            List<COMPRAS> data2 = (List<COMPRAS>)HttpContext.Current.Session["data2"];
            HttpContext.Current.Session["data1"] = null;
            HttpContext.Current.Session["data2"] = null;
            ...
            if (data1 != null && data2 != null)
            {
                ...
                using (IDbConnection con = new SqlConnection(Extensions.GetBD_CFDI()))
                {
                    ...
                    WHERE xml.ID = @ID";
                        ...
                        var ws = wb.Worksheets.Add("Factura-Albaran");
                        ...
                        if( data2.Count > 0 )
                        {
                            ...
                            var total = ran.LastRow().RowBelow();
                            total.Cell("C").Value           = "TOTAL:";
                            total.Cell("C").Style.Font.Bold = true;

                            total.Cell("D").Value = data2.Sum(s => s.TOTAL);
                            total.Cell("D").Style.NumberFormat.Format = "$#,##0.00";

                            if (ran != null)
                            {
                                ran.Columns("B").Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Left;
                                ran.Columns("D").Style.NumberFormat.Format = "$#,##0.00";
                                ran.AdjustToContents();
                            }
                        }
                        ...
                        Response.AddHeader(
                                "content-disposition",
                                "attachment;filename=\"Factura_Albaran_"
                              + Extensions.GetAlias()
                              + ".xlsx\"");
        ...
        public static void SetID(List<int> data1, List<COMPRAS> data2)
        {
            HttpContext.Current.Session["data1"]   = data1;
            HttpContext.Current.Session["data2"]   = data2;
        }
    }
}