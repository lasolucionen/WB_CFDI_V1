    ...
    public partial class Pag5 : Page
    {
        ...
        public static string GetFlag(CFDI_ACCOUNT ac)
        {
            ...
            try
            {
                ...
                using (IDbConnection con = new SqlConnection(Extensions.GetBD_CFDI()))
                {
                    string sql1 = @"SELECT TOP 1 USER_ID FROM CFDI_XML_PSI WHERE USER_ID = @USER_ID";

                    var rs1 = con.Query<CFDI_XML>(sql1, new { USER_ID = ac.ID}).SingleOrDefault();
        ...
        public static string Agregar(CFDI_ACCOUNT ac)
        {
            ...
            try
            {
                ...
                using (IDbConnection con = new SqlConnection(Extensions.GetBD_CFDI()))
                {
                    ...
                    string sql1 = @"SELECT ID FROM CFDI_ACCOUNT WHERE USER_NAME = @USER_NAME";
                    ...
                    DECLARE @XID INT;
                    SET @XID = (SELECT MAX(p.ID) FROM CFDI_ACCOUNT p);
                    SET @XID = ISNULL( @XID , 0 ) + 1;

                    INSERT CFDI_ACCOUNT (ID, USER_NAME, PASSWORD, ROLE)
                                 VALUES (@XID, @USER_NAME, @PASSWORD, @ROLE)";
        ...
        public static string GetRole(bool Todos)
        {
            ...
            try
            {
                ...
                using (IDbConnection con = new SqlConnection(Extensions.GetBD_CFDI()))
                {
                    var rs = con.Query<CFDI_ROLE>("SELECT * FROM CFDI_ROLE ORDER BY ID").ToList();
            ...
            try
            {
                ...
                using (IDbConnection con = new SqlConnection(Extensions.GetBD_CFDI()))
                {
                    ...
                    SELECT
                      ...
                      a.ROLE,
                      b.ROLE_NAME
                    FROM dbo.CFDI_ACCOUNT a
                    INNER JOIN dbo.CFDI_ROLE b ON a.ROLE = b.ID
                    WHERE 
