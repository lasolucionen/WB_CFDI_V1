        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (data != null && chkPDF != null )
            {
                ...
                using (IDbConnection con = new SqlConnection(Extensions.GetBD_CFDI()))
                {
                    ...
                    LEFT OUTER JOIN " + emp.ICG_BD + @".dbo.IT_RELFACTURAS_COMPRAS ic WITH (NOLOCK) ON xml.UUID = ic.FOLIOFISCAL
                    WHERE xml.ID = @ID";
                        ...
                        using (ZipStorer zip = ZipStorer.Create(st, ""))
                        {
                            ...
                            {
                                var rs = con.Query<CFDI_XML>(sql, new {ID = it}).FirstOrDefault();
                            ...
                            total.Cell("G").Value = "TOTAL:";
                            total.Cell("G").Style.Font.Bold = true;

                            total.Cell("H").Value = ls.Sum(s => s.TOTAL);
                            total.Cell("H").Style.NumberFormat.Format = "$#,##0.00";
                            ...
                            if (ran != null)
                            {
                                ...
                                ran.Columns("L").Style.NumberFormat.Format = "$#,##0.00";
