                        ...
                        string sql = @"
                            ...
                            SELECT
                              ...
                              --SUM(b.TOTAL) AS IMPORTE
                                ...
                                SUM(
                              case 
                                  when b.TIPO_COMPROBANTE IS NULL then b.TOTAL
                                  when LOWER(b.TIPO_COMPROBANTE) = 'e' then b.TOTAL*-1
                                  when LOWER(b.TIPO_COMPROBANTE) = 'egreso' then b.TOTAL*-1
                                  ELSE b.TOTAL
                              end ) AS IMPORTE


                            FROM dbo.CFDI_XML_PSI b WITH (NOLOCK)
                            INNER JOIN dbo.CONTRA_RECIBOS_PSI a WITH (NOLOCK) ON b.CONTRA_RECIBO_ID = a.ID
                            WHERE
